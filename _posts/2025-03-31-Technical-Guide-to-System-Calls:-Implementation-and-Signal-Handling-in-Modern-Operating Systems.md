...
The signal processing workflow in the kernel follows these steps:

1. **Signal Generation**: A signal is generated by another process, the kernel, or hardware
2. **Signal Delivery**: The kernel marks the signal as pending for the target process
3. **Signal Processing**: Before returning to user space, the kernel checks for pending signals
4. **Handler Execution**: If a handler is registered, the kernel sets up the user stack to execute it
5. **Return Handling**: After the handler completes, the kernel decides whether to restart the system call
...
### 1. Fully Synchronous (Fast) System Calls {#fully-synchronous-fast-system-calls}
...
### 2. Asynchronous System Calls {#asynchronous-system-calls}
...
### 3. Blocking with Timeout System Calls {#blocking-with-timeout-system-calls}
...
### 4. Indefinitely Blocking System Calls {#indefinitely-blocking-system-calls}
...
### 5. Restartable System Calls {#restartable-system-calls}
...
The distinction between fast and slow system calls is fundamental to understanding how operating systems handle process scheduling, signal delivery, and I/O operations.
